    ...
    override fun onCreateView(
        ...
        return ComposeView(requireContext()).apply {
            ...
            setContent {
                ...
                var autoStartWifi by remember { mutableStateOf(Prefs.getAutoStartWifi(context)) }
                var autoStopCellular by remember { mutableStateOf(Prefs.getAutoStopCellular(context)) }
                var autoStopMetered by remember { mutableStateOf(Prefs.getAutoStopMetered(context)) }
                SettingsApp(
                    style = style,
                    ...
                    autoStartWifi = autoStartWifi,
                    autoStopCellular = autoStopCellular,
                    autoStopMetered = autoStopMetered,
                    onPageTransitionChange = { i ->
                        transition = i
                        Prefs.setPageTransition(context, i)
                        (requireActivity() as? MainActivity)?.applyPageTransition()
                    },
                    onAutoStartWifiChange = { v ->
                        autoStartWifi = v
                        Prefs.setAutoStartWifi(context, v)
                        NetworkAutoControl.evaluate(context)
                    },
                    onAutoStopCellularChange = { v ->
                        autoStopCellular = v
                        Prefs.setAutoStopCellular(context, v)
                        NetworkAutoControl.evaluate(context)
                    },
                    onAutoStopMeteredChange = { v ->
                        autoStopMetered = v
                        Prefs.setAutoStopMetered(context, v)
                        NetworkAutoControl.evaluate(context)
                    },
                )
            }
